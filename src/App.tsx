  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        if (rawData.length === 0) {
          throw new Error(
            'Parsed data is empty. Please check the file content.',
          );
        }
      ...
      return (
        <div className="max-w-3xl mx-auto pt-10 animate-in fade-in duration-300">
          <Card
            ...
            description="Supports Structured & Binary Formats"
          >
              ...
              {isUploading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    Supported: .csv, .tsv, .xlsx, .json, .parquet, .feather,
                    .h5
                  </p>
      ...
      return (
        <div className="max-w-5xl mx-auto pt-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-8 text-center text-slate-500">
            {cleaningRun ? (
              <div
                className="text-left cursor-pointer hover:bg-slate-50 p-4 rounded border mb-2"
                onClick={() => {
                  setActiveDataset(
                    datasets.find((d) => d.id === cleaningRun.datasetId) || null,
                  );
                  setView('dashboard');
                }}
              >
        ...
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 flex items-center">
              {activeDataset ? (
                <>
                  {(activeDataset.name.endsWith('xlsx') ||
                    activeDataset.name.endsWith('xls')) && (
                    <FileSpreadsheet
            ...
            {activeDataset && (
              <p className="text-sm text-slate-500">
                ...
                • {activeDataset.rowCount} Rows • {activeDataset.colCount}{' '}
                Columns
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col space-y-8">
            ...
            <section>
              <div className="flex items-center space-x-2 mb-4">
                <div className="h-6 w-6 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-sm">
                  1
                </div>
            ...
            {cleaningRun && (
              <section id="results">
                <div className="flex items-center space-x-2 mb-4">
                  <div className="h-6 w-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center font-bold text-sm">
                    3
                  </div>
                  <h3 className="text-lg font-semibold text-slate-800">
                    Results & Suggestions
